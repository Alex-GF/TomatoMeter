...
const Sidebar = ({ selectedPage, setSelectedPage }: { selectedPage: string, setSelectedPage: (page: string) => void }) => {
  ...
  const activeAddons = currentSubscription.slice(1).map(str => {
    ...
    <div className="flex h-full min-w-72 flex-col items-center bg-gray-900 px-4 py-10 text-white">
      <div className="mb-8 text-center">
        ...
        <motion.div
          ...
          className={`mt-4 rounded-2xl p-0.5 bg-gradient-to-r ${PLAN_COLORS[planName] || PLAN_COLORS['FREE']} shadow-xl`}
        >
          <div className="flex flex-col items-center rounded-2xl bg-gray-900/90 px-5 py-4 min-w-[180px]">
            ...
            <AnimatePresence>
              {activeAddons.length > 0 && (
                ...
                >
                  <span className="text-xs font-semibold text-purple-100 mb-1">Active Add-ons:</span>
                  <ul className="flex flex-wrap justify-center gap-1">
                    {activeAddons.map(([name, v], idx) => (
                      ...
                      >
                        {name}{v > 1 ? <span className="ml-1 text-yellow-200">×{v}</span> : null}
                      </motion.li>
                    ))}
                  </ul>
                </motion.div>
