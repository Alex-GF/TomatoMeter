      ...
      <div className="flex flex-col gap-6">
        {settingsOptions.map((option, idx) => (
          ...
          >
            ...
            <motion.button
              whileTap={{ scale: 0.85 }}
              className={`relative w-14 h-8 flex items-center rounded-full p-1 transition-colors duration-300 ${toggles[option.name] ? 'bg-purple-500' : 'bg-gray-300'} ${option.premium ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
              onClick={() => !option.premium && handleToggle(option.name)}
              disabled={option.premium}
            >
              <motion.span
                layout
                className={`absolute left-1 top-1 w-6 h-6 rounded-full bg-white shadow-md transition-transform duration-300 ${toggles[option.name] ? 'translate-x-6' : ''}`}
                animate={{ x: toggles[option.name] ? 24 : 0 }}
